...
export default function ViewerPage() {
    ...
    const [standaloneQuestion, setStandaloneQuestion] = useState<any>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // 지문 없으면 단독 문제 ID로 재시도 (grammar 페이지에서 passageId 대신 questionId가 들어온 경우)
                const qRes = await fetch(`/api/question?id=${encodeURIComponent(passageId)}`);
                if (qRes.ok) {
                    const qData = await qRes.json();
                    if (qData.passage) {
                        // 사실은 지문이 있는 문제 — 해당 지문 화면으로 전환
                        const pRes = await fetch(`/api/passage?id=${qData.passage.id}`);
                        if (pRes.ok) setPassage(await pRes.json());
                        else setStandaloneQuestion(qData);
                    } else {
                        setStandaloneQuestion(qData);
                    }
                }
            } catch (e) {
    ...
    if (!passage && !standaloneQuestion) {
        return (
            <div className="h-screen flex flex-col items-center justify-center bg-slate-50 p-6">
                <AlertTriangle className="w-16 h-16 text-amber-400 mb-4" />
                <h2 className="text-2xl font-black text-slate-800 mb-2">문항을 찾을 수 없습니다.</h2>
    ...
    if (standaloneQuestion) {
        const q = standaloneQuestion;
        const tagNames: string[] = (q.tags || []).map((t: any) => t.tag?.name).filter(Boolean);
        return (
            <div className="flex flex-col h-screen bg-slate-100 overflow-hidden font-sans">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="font-bold text-slate-900 flex items-center gap-2">
                                ...
                                {q.year ? `${q.year}년 ` : ''}{q.month ? `${q.month}월 ` : ''}{q.grade ? `${q.grade}학년 ` : ''}{q.area || ''}
                            </h1>
                            <p className="text-xs text-slate-500 font-medium tracking-tight">단독 문제 (지문 없음)</p>
                        </div>
                    </div>
                ...
                <main className="flex-1 overflow-y-auto bg-slate-50 p-10 custom-scrollbar">
                        ...
                        <div className="bg-white rounded-[40px] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                            ...
                            <div className="p-10">
                                <div className="mb-10 rounded-3xl overflow-hidden border border-slate-100 bg-white p-2 shadow-inner">
                                    <img src={q.imageUrl} alt="Question" className="w-full h-auto rounded-2xl" />
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-6">
                                    <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 flex flex-col gap-4">
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div className="flex-grow">
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter mb-2">연관 태그</p>
                ...
                <style jsx global>{`
                    ...
                    .custom-scrollbar::-webkit-scrollbar-thumb:hover {
                        background: #cbd5e1; background-clip: content-box;
                    }
                `}</style>
            </div>
        );
